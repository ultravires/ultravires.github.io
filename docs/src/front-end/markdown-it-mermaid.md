...
export default async function markdownItMermaid(options: MermaidConfig = {}) {
  const plugin = (md: MarkdownIt) => {
    ...
    md.renderer.rules.fence = function (tokens: Token[], idx: number, options: Options, env: any, self: Renderer) {
      ...
      return (
        _fence?.(tokens, idx, options, env, self) ??
...
```javascript
async function useMarkdownItMermaid() {
  md.use(await markdownItMermaid());
}
```
