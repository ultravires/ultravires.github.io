...
<template>
  <ul class="group/outline" :class="{ relative: root }">
    <li v-for="{ children, link, title } in headers" :key="link">
      <a
        :href="link"
        class="group/link hover:text-primary [&.active]:text-lg [&.active]:text-primary [&.active]:font-medium block text-normal text-opacity-60 h-[32px] leading-[32px] whitespace-nowrap text-ellipsis overflow-hidden transition-all duration-300"
        :title="title"
        @click="onClick"
      >
