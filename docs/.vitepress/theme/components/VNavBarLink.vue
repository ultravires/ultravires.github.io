...
<template>
  ...
  >
    ...
    >
      <div
        class="dark:bg-neutral-900 flex items-center gap-2 p-2 mt-3 bg-white bg-opacity-90 border border-solid border-primary rounded-full text-base tracking-wide cursor-default backdrop-blur-md"
      >
        <VLink
          v-for="item in item.items"
          class="dark:hover:text-reverse hover:bg-primary hover:px-5 hover:text-reverse relative px-4 py-2 decoration-none rounded-full whitespace-nowrap text-base transition-all duration-300"
          :class="{
            'after:absolute after:w-3 after:h-1 after:bg-primary after:rounded-full after:bottom-0 after:left-1/2 after:-translate-x-1/2 active relative':
              isActive(
                page.relativePath,
